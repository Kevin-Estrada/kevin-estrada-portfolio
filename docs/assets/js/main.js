  ...
  for (let x in response) {
    ...
    if (Object.keys(response[x])[0] == 'langAltText') {
      ...
      if (response[x].linkToProject == 'NA') {
        newNodeThree.innerHTML = `<div class="card shadow"><div class="card-body"><h5 class="card-title text-decoration-underline">${response[x].NameOfProject}</h5><p class="card-text">${response[x].descOfProject}</p><a href="./pages/project-not-found.html" class="btn btn-primary text-white" style="margin-right: 1rem;">Project Link</a><a href="${response[x].linkToProjectRepo}" target="_blank" class="btn btn-secondary text-white">Repo Link</a></div></div>`;
      } else {
